...
# Allow running this file directly (`streamlit run app/streamlit_app.py`)
# without requiring the deepscan package to be pip-installed first.
_REPO_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
for _path in (os.path.join(_REPO_ROOT, "src"), _REPO_ROOT):
    if _path not in sys.path:
        sys.path.insert(0, _path)

from deepscan.inference import Predictor  # noqa: E402
from scripts.download_model import ensure_model  # noqa: E402
...
def load_predictor() -> Predictor:
    model_path = ensure_model(MODEL_FILENAME, dest_dir=_REPO_ROOT)
...
def run_analysis(image: Image.Image):
    ...
    metrics = {
        "Fake Prob":  f"{pred.fake_prob:.2f}",
        "Real Prob":  f"{pred.real_prob:.2f}",
        "Confidence": f"{max(pred.fake_prob, pred.real_prob):.2f}",
        "Verdict":    "FAKE" if pred.is_fake else "REAL",
        "Model":      pred.backbone,
        "Input Size": f"{predictor.img_size}×{predictor.img_size}",
    }

    # Only report signals the model actually computed — no hardcoded
    # "no copy-move forgery detected" style claims the pipeline never checks.
    if pred.is_fake:
        flags = [
            ...
            ("info", f"Fake-class confidence: {pred.fake_prob:.0%}"),
        ]
    else:
        flags = [
            ("ok",   "Model predicts this image is authentic"),
            ("ok",   "Probability distribution favors the real class"),
            ("info", f"Real-class confidence: {pred.real_prob:.0%}"),
        ]
    if not pred.labels_verified:
        flags.append((
            "warn",
            "Real/Fake label order for this checkpoint is unverified — see README before trusting verdicts",
        ))

    return pred, metrics, flags


# ─── Hero ────────────────────────────────────────────────────────────────────
st.markdown("""
<div class="hero">
    <div class="hero-badge">🔬 Powered by Deep Learning + GradCAM</div>
    ...
    <div class="stats-row">
        <div class="stat-pill"><span class="stat-dot"></span> <b>CNN</b> backbone</div>
...
if uploaded_file is not None:
    ...
    with col_img:
        ...
        st.markdown('<div class="section-desc">Input image — resized for the model</div>', unsafe_allow_html=True)
    ...
    with col_info:
        ...
        st.markdown('<div class="section-desc">The model will run a forward pass and compute a GradCAM heatmap on its final convolutional block.</div>', unsafe_allow_html=True)
        st.markdown("""
        <ul class="analysis-list">
            <li class="analysis-item">
                <span class="analysis-icon icon-info">①</span>
                The CNN backbone extracts deep visual features
            </li>
            <li class="analysis-item">
                <span class="analysis-icon icon-info">②</span>
            ...
            <li class="analysis-item">
                <span class="analysis-icon icon-info">③</span>
                GradCAM backpropagates gradients to highlight influential regions
            </li>
            <li class="analysis-item">
                <span class="analysis-icon icon-info">④</span>
    ...
    if run:
        with st.spinner("Running inference + GradCAM…"):
            pred, metrics, flags = run_analysis(image)
        is_fake, fake_prob, real_prob, gradcam_img = pred.is_fake, pred.fake_prob, pred.real_prob, pred.gradcam_image
        ...
        with tab1:
            ...
            <div class="section-desc" style="margin-top:1rem;">
                🔴 Red / warm areas = regions driving the predicted verdict &nbsp;·&nbsp;
                🟢 Green / cool = low activation
            </div>
            """, unsafe_allow_html=True)
        ...
        with tab2:
            ...
            with st.expander("ℹ  About this model"):
                ...
                <div style="font-family:'DM Mono',monospace;font-size:0.75rem;color:#9d9ab0;line-height:1.8;">
                    <b style="color:#a78bfa">Architecture:</b> {pred.backbone} fine-tuned on a custom deepfake dataset<br>
                    <b style="color:#a78bfa">Input size:</b> {metrics['Input Size']} RGB<br>
                    <b style="color:#a78bfa">Classes:</b> label order {'verified' if pred.labels_verified else 'UNVERIFIED — see README'}
                </div>
                """, unsafe_allow_html=True)
...
<div class="how-section">
    ...
    <div class="how-grid">
        <div class="how-card">
            ...
            <div class="how-desc">A convolutional backbone encodes spatial and texture features through progressive layers, building a rich visual representation.</div>
        </div>
        <div class="how-card">
            <div class="how-num">02</div>
            <div class="how-title">Classification</div>
            <div class="how-desc">A custom classifier head outputs probability scores for REAL and FAKE classes.</div>
        </div>
        <div class="how-card">
...
st.markdown("""
<div class="footer">
    DeepScan · GradCAM · Built with Streamlit
</div>
""", unsafe_allow_html=True)
